...
5、安装包版：收到合同款项后，乙方将 license、version 和使用手册发送到甲方指定邮箱，同时提供专业技术支持服务。开具增值税专用发票或普票。   
...
乐吾乐2D可视化后端源码语言是golang，主要用于文件保存，详细功能参考[API 接口文档](./commercial-api) 最下面的“后端模块”介绍，如不购买后端源码，用户也可以参考API接口文档自己实现。
